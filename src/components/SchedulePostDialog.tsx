...
import { schedulePostSchema, type SchedulePostFormData } from "@/stores/scheduler/scheduler.dto";
...
export function CreateScheduleDialog({ initialDate }: CreateScheduleDialogProps) {
  ...
  const currentMaxLimit = watchPlatforms.length > 0 
    ? Math.min(...watchPlatforms.map(p => PLATFORM_LIMITS[p.toLowerCase()] || PLATFORM_LIMITS.default))
     ...
     <DialogContent className="sm:max-w-[500px] bg-white p-0 overflow-hidden flex flex-col max-h-[95vh]">
        ...
        <DialogHeader className="p-6 pb-2">
          <DialogTitle className="text-xl">
            Schedule Post for {initialDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-2 custom-scrollbar">
          ...
          <Form {...form}>
            <form id="schedule-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-col justify-end items-end pb-2">
                  <span className={cn(
                    ...
                    isOverLimit ? 'text-red-500 font-bold bg-red-50' : 'text-muted-foreground'
                  )}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-[11px] font-bold uppercase text-muted-foreground tracking-wider">Content</FormLabel>
                    <FormControl>
                      <Textarea 
              ...
              <FormField
                ...
                render={() => (
                  <FormItem>
                    <FormLabel className="text-[11px] font-bold uppercase text-muted-foreground tracking-wider">Target Platforms</FormLabel>
                    <div className="flex flex-wrap gap-2">
                      {connectedAccounts?.map((account) => {
                        const isSelected = watchPlatforms.includes(account.platform);
                        return (
                          <Button
                            key={account.id}
                            type="button"
                            ...
                            className={cn(
                              ...
                              isSelected ? "bg-violet-600 hover:bg-violet-700 shadow-sm shadow-violet-100" : "hover:border-violet-200"
                            )}
                            onClick={() => {
                              const next = isSelected 
                                ? watchPlatforms.filter(p => p !== account.platform)
              ...
              <div className="space-y-2 pb-4">
                <FormLabel className="text-[11px] font-bold uppercase text-muted-foreground tracking-wider">Media</FormLabel>
                {!watchImage ? (
                  <div className="relative border-2 border-dashed border-stone-200 rounded-xl p-6 flex flex-col items-center justify-center bg-stone-50/50 hover:bg-stone-50 hover:border-violet-300 transition-all cursor-pointer group">
                    ...
                    {uploading ? <Loader2 className="animate-spin h-5 w-5 text-violet-600" /> : <Upload className="h-5 w-5 text-stone-400 group-hover:text-violet-500 transition-colors mb-1" />}
                    <p className="text-[10px] text-stone-500 font-medium">Click to upload (Max 5MB)</p>
                  </div>
                ) : (
                  <div className="relative group overflow-hidden rounded-xl border border-stone-200">
                    <img src={watchImage} className="w-full h-32 object-cover" alt="Preview" />
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                       <button
                        type="button"
                        onClick={() => { form.setValue("imageLink", ""); form.setValue("imageMimeType", ""); }}
        ...
        <div className="p-6 pt-4 border-t bg-stone-50/30">
          <Button 
            ...
            disabled={form.formState.isSubmitting || uploading || isOverLimit || isLimitReached}
          >
            {form.formState.isSubmitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "Schedule Post"}
          </Button>
        </div>
